  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className={textColor}>{allOk ? "All Connected" : allDown ? "Offline" : "Partial"}</span>
      </button>
      ...
      {expanded && (
        <div className="absolute bottom-full right-0 mb-2 w-72 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 overflow-hidden">
          <div className="px-4 py-3 border-b bg-[#f4f6f9]">
            <div className="font-bold text-sm text-[#2c3e50]">🔌 Server Status</div>
            <div className="text-[10px] text-gray-400 mt-0.5">Last checked: {lastCheck || "—"}</div>
          </div>
          <div className="px-4 py-3 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-gray-500">Master Data</span>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-gray-500">Auth & DB</span>
            ...
            <div className="text-[10px] text-gray-500 space-y-1">
              {health && <div>Version: <span className="font-mono font-bold">{health.version}</span> • WIB: {health.wib}</div>}
              {!health && <div className="text-red-500">⚠ API unreachable — backend may be down</div>}
            </div>
            <button onClick={checkHealth} className="w-full text-[10px] py-1 bg-gray-100 rounded hover:bg-gray-200 text-gray-500">🔄 Refresh Status</button>
          </div>
        </div>
      )}
    </div>
